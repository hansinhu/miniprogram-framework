...
import { REPLACE, PROPS, TEXT, REORDER } from "../util/common.js";
...
function dfsWalk(oNode, nNode, index, patches) {
  ...
  if (isString(oNode) && isString(nNode)) {
    oNode !== nNode && currentPatch.push({ type: TEXT, content: nNode });
    // 同种标签并且key相同
    ...
    if (!(!isNotEmptyObj(nNode.props) && nNode.props.hasOwnProperty("ignore"))) {
      (oNode.children.length || nNode.children.length) && diffChildren(oNode.children, nNode.children, index, patches, currentPatch);
    }
  } else {
    // 都不符合上面情况就直接替换
